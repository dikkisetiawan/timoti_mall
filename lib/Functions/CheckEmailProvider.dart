...
import 'package:timoti_project/Api/CheckEmailExist-Api/Fetch-CheckEmailExist-Api.dart';
import 'package:timoti_project/Data-Class/CheckEmailResult.dart';
...
Future<CheckEmailResult?> checkEmailExistenceResult(SignInType errorType, String email,) async {
  ...
  await fetchCheckEmailApi(email).then((value){
    if(value.isExist != null){
      ...
      if(isEmailExist == true){
        int savedIndex = -1; // <-- Use for indexing random provider as final result
        if(value.provider != null){
          for (int i = 0; i <  value.provider!.length; ++i) {
            if (convertStringToSignInType(value.provider![i]) == errorType) {
              ...
              if(convertStringToSignInType(value.provider![i]) != SignInType.Phone){
                savedIndex = i;
              }
            }
            if (i == value.provider!.length - 1) {
              ...
              if(savedIndex != -1){
                return result = new CheckEmailResult(
                  ...
                  targetProvider: convertStringToSignInType(value.provider![savedIndex]),
                );
              }
              /// Phone, error
              else{
