...
class AddressEditPage extends StatefulWidget {
  static const routeName = '/_AddressEditPage';
  @override
...
class _AddressEditPageState extends State<AddressEditPage> {
  ...
  void didChangeDependencies() {
    if (loaded == false) {
      print("=== Address Edit Page =======");

      /// Assign Data
      data = ModalRoute.of(context)?.settings.arguments as AddressEditArgument;

      /// Apply to text controller
      _fullNameController.text = data.addressClass.fullName as String;
      _phoneController.text = data.addressClass.phone as String;
      _addressDetailController.text =
          data.addressClass.addressDetails as String;
      _postcodeController.text = data.addressClass.postcode as String;
      _cityController.text = data.addressClass.city as String;
      currentState = data.addressClass.state as String;
      _addressLabelController.text = data.addressClass.label as String;
      addressLatitude = data.addressClass.latitude as double;
      addressLongitude = data.addressClass.longitude as double;
      print("Data Retrieved (Latitude): " + addressLatitude.toString());
      print("Data Retrieved (Longitude): " + addressLongitude.toString());

      if (data.addressClass.index == 0) {
        isDefault = true;
      }
                ...
                SafeArea(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      //             initLatLong: LatLng(addressLatitude, addressLongitude),
                      //           ),
                      //         ),
                      //       );
                      //
  ...
  /// Update Address
  void updateAddress(DeviceDetails _deviceDetails) async {
    loading = true;
    setState(() {});
    ...
    if (userData["Address"] != null) {
      ...
      if (userData["Address"].length > 0) {
        ...
        for (int i = 0; i < userData["Address"].length; ++i) {
          int targetIndex = data.addressClass.index as int;

          /// Current index reach target index
          if (i == targetIndex) {
          ...
          /// No Affect on other index
          else {
            /// Assign Data
            AddressClass tempAddress = new AddressClass(
              addressDetails: userData["Address"][i]["Address_Details"],
              state: userData["Address"][i]["State"],
              city: userData["Address"][i]["City"],
              country: userData["Address"][i]["Country"],
              fullName: userData["Address"][i]["Full_Name"],
              label: userData["Address"][i]["Label"],
              phone: userData["Address"][i]["Phone"],
              postcode: userData["Address"][i]["Postcode"],
              latitude: userData["Address"][i]["Latitude"],
              longitude: userData["Address"][i]["Longitude"],
            );
          ...
          if (i == userData["Address"].length - 1) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: _getCustomAppBar("Edit Address", _widgetSize, _deviceDetails),
  ...
  List<Widget> getPageContent(
    ...
    /// Sign up
    pageContent.add(
      getUpdateButtonUI(_deviceDetails, _widgetSize),
    );
